    ...
    return BlocProvider(
      ...
      child: BlocListener<ScanBloc, ScanState>(
        listener: (context, state) {
          // Cuando el ticket se procesa exitosamente,
          // recargar los tickets en el dashboard
          if (state is ScanProcessSuccess) {
        ...
        child: Scaffold(
          ...
          body: BlocBuilder<ScanBloc, ScanState>(
            builder: (context, state) {
              ...
              if (state is ScanProcessSuccess) {
                final ticketData = state.response.data;
                return SingleChildScrollView(
                  padding: const EdgeInsets.all(16.0),
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.stretch,
                        ...
                        Card(
                          child: Padding(
                            ...
                            child: Column(
                              ...
                              children: [
                                Text(
                                  'Tienda: ${ticketData.store ?? "N/A"}',
                                  style: Theme.of(
                                    context,
                                  ).textTheme.titleMedium,
                                ),
                                if (ticketData.transactionId != null) ...[
                                  const SizedBox(height: 8),
                                  Text(
                                ...
                                if (ticketData.finalTotal != null) ...[
                                  const SizedBox(height: 8),
                                  Text(
                                    'Total: €${ticketData.finalTotal!.toStringAsFixed(2)}',
                                    style: Theme.of(
                                      context,
