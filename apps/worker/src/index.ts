...
async function assertQueueWithErrorHandling(
    ...
    try {
        ...
        if (
            ...
            console.error(
                `    rabbitmqctl delete_queue ${RETRY_QUEUE_NAME}`
            );
            console.error(
                `    rabbitmqctl delete_queue ${DLQ_FINAL_NAME}`
            );
            console.error(
    ...
    await channel.consume(
        ...
        async (msg: amqp.ConsumeMessage | null) => {
            ...
            try {
                ...
                const retryCount = (msg.properties.headers?.["x-retries"] as number) || 0;
                const maxRetries = env.MAX_RETRIES;

                if (retryCount >= maxRetries) {
                    // Send to final DLQ
                    await handleFailedMessage(currentChannel, msg, RETRY_QUEUE_NAME, DLQ_FINAL_NAME);
