...
export const TicketDataSchema = z.object({
  ...
  date: z
    .string(), // Date is always present (from ticket or current date), format can vary but we normalize it
  time: z
    .string()
    .nullable()
    .optional(), // Time format can vary, we normalize it but accept any string
...
/**
 * Schema for validating file upload response
 */
...
/**
 * Schema for validating ticket processing response
 */
export const TicketProcessingResponseSchema = z.object({
  success: z.boolean(),
  data: TicketDataSchema,
});

export type TicketProcessingResponse = z.infer<
  typeof TicketProcessingResponseSchema
>;

/**
 * Validates ticket data from Gemini response
 */
...
/**
 * Safely validates ticket data, returns null if invalid
 */
export function safeValidateTicketData(data: unknown): TicketData | null {
  const result = TicketDataSchema.safeParse(data);
